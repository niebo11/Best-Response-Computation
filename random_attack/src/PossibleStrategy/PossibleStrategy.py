...
def getTargetRegion(G, Component):
    G_ini = G.to_undirected()
    G_ini.remove_nodes_from([node for node in G.nodes if not G_ini.nodes[node]['target'] and node in Component])
...
def Utility(G, C, T, targetRegions, max_T, Targeted, currentSize, Cinc):
    total = 0.0
    if len(targetRegions) > 0:
        ...
        for item in targetRegions:
            if (Targeted and set(Cinc).isdisjoint(item)) or not Targeted:
                ...
                total += len(item) / max_T * result
        if T:
            total += probabilityNotAttacked * len(C)
    else:
        if T:
            if Targeted:
                total = (1 - currentSize / max_T) * len(C)
            else:
                total = len(C)
        else:
            total = 0
...
def possibleStrategy(G, M, Imm, Ci, Cinc, alpha, max_T, currentSize, Targeted):
    B = []
    for C in Ci:
        B += partnerSetSelect(G.subgraph(C), C, Cinc, alpha, max_T, currentSize, Targeted)
    return M + B, Imm


def partnerSetSelect(G, CI, Cinc, alpha, max_T, currentSize, Targeted):
    targetRegions = getTargetRegion(G, CI)
    tempt = [item for item in Cinc if item in CI]
    empty = Utility(G, CI, tempt, targetRegions, max_T, currentSize, Targeted, Cinc)
    CImm = [item for item in CI if G.nodes[item]['immunization']]
    single_edge = {
        item: (Utility(G, CI, list(set([item] + tempt)), targetRegions, max_T, currentSize, Targeted, Cinc) - alpha)
    ...
    if len(CImm) > 1:
        ...
        G2, I, collapse_dict = collapse_graph(G1)
        if len([item for item in G2.nodes if G2.nodes[item]['immunization']]) > 1:
            ...
            if len([item for item in M1.nodes if M1.nodes[item]['immunization']]) > 1:
                opt = MetaTreeSelect(M1, Cinc, alpha, max_T)
                opt = initial_opt(opt, mappingG, mappingM)

                multiple_edge = {tuple(item): (Utility(G, CI, item + tempt, targetRegions, max_T, currentSize, Targeted,
